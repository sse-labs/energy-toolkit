...
class Config_Parser:
    ...
    def validate(data: any) -> int:
        ...
        if len(programs) == 0:
            raise click.ClickException("'programs' list must contain at least one entry.")

        # Validate each program
        for idx, program in enumerate(programs):
            if not isinstance(program, dict):
                raise click.ClickException(f"Entry {idx} in 'programs' must be a dictionary.")
            ...
            if not isinstance(exe_path, str):
                raise click.ClickException(f"'executeable' in entry {idx} must be a string.")

            # Check if the executable exists
            if not os.path.isfile(exe_path):
                raise click.ClickException(f"Executable '{exe_path}' in entry {idx} does not exist.")
